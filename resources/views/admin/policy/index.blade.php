                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <a class="btn btn-success"
                                   href="{{route('admin.policy.create')}}">Thêm chính sách</a>
                            </div>
                            @if(count($listData) > 0)
                                <table class="table">
                                    <thead>
                                    <tr>
                                        <th scope="col">#</th>
                                        <th scope="col" style="width: 300px">Tên chính sách</th>
                                    ...
                                    @foreach($listData as $key => $value)
                                        <tr>
                                            ...
                                            <td>{{$value->name}}</td>
                                            <td class="text-center">{{$value->index}}</td>
                                            <td>
                                                <div class="btn-group">
                                                    <a href="{{url('admin/policy/edit/'.$value->id)}}"
                                                       class="btn btn-icon btn-light btn-hover-success btn-sm"
                                                       data-bs-toggle="tooltip" data-bs-placement="top" title=""
                                                       data-bs-original-title="Cập nhật">
                                                        <i class="bi bi-pencil-square "></i>
                                                    </a>
                                                    <a href="{{url('admin/policy/delete/'.$value->id)}}"
